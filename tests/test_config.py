...
class ConfigValidationTests(unittest.TestCase):
    def test_sections_validate_themselves_on_construction(self):
        ...
        for build in [lambda: AuthConfig('127.0.0.1', 70000),
...
class ConfigTests(unittest.TestCase):
    ...
    def test_since_parses_after_overrides_and_interpolation(self):
        config = from_mapping(complete_mapping(), Path('/tmp'), overrides=[
            'sync.since=2026-09-16T10:00:00+02:00'])
        ...
        self.assertEqual(from_mapping(data, Path('/tmp')).sync.since,
    ...
    def test_missing_settings_and_sections_fail(self):
        for section, key in [('auth', 'port'), ('sync', 'since'), ('sync', 'include_private'), ('strava', 'client_id'),
                             ('strava', 'client_secret'), ('telegram', 'bot_token'),
                             ('telegram', 'channel_id'), ('logging', 'level')]:
            data = complete_mapping()
    ...
    def test_unknown_fields_invalid_types_and_ranges(self):
        for data in [{'auth': {'port': 'not-a-number'}},
                     {'auth': {'port': 0}},
    ...
    def test_arbitrary_nested_overrides_and_precedence(self):
        ...
        config = from_mapping(data,
                              Path('/tmp'), overrides=[
                              'telegram.channel_id=-100456', 'auth.port=9000',
                              'storage.database=other.db', 'sync.since=null'])
    ...
    def test_interpolation_and_last_override_wins(self):
        config = from_mapping(complete_mapping(), Path('/tmp'), overrides=[
            'auth.port=9000', 'auth.port=9001', 'strava.client_id=${auth.port}'])
    ...
    def test_bad_override_syntax_and_unknown_fields(self):
        for override in ['sync.since', '=10', 'sync..since=10',
                         'sync.typo=10', 'auth.port=nope']:
    ...
    def test_yaml_example_and_duplicate_keys(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for text in ['auth:\n  port: 30\n  port: 60\n',
                         '!!python/object/apply:os.system ["exit 1"]', 'sync: [broken']:
